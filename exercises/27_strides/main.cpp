...
// 通常来说，形状为 [d0, d1, ..., dn] 的张量，第 n 维是 dn 个连续的元素，第 n-1
// 维是 dn-1 个连续的 dn 个元素，以此类推。
// 张量的步长或跨度指的是张量每个维度上坐标 +1 时，数据指针跨过的范围。
// 因此，一个连续张量，其第 n 维的步长为 1，第 n-1 维的步长为 dn，第 n-2
// 维的步长为 dn*dn-1，以此类推。 例如，一个 2x3x4 张量，其步长为 [12, 4, 1]。
...
std::vector<udim> strides(std::vector<udim> const &shape) {
  ...
  strides.back() = 1;
  int index = 1;
  for (auto item = strides.rbegin() + 1; item != strides.rend(); item++, index++) {
    *item = *(item -1) * shape[shape.size()-index];
  }
  // READ: 逆向迭代器 ststrides.rbegin()+id::vector::rbegin
  // <https://zh.cppreference.com/w/cpp/container/vector/rbegin>
  //       使用逆向迭代器可能可以简化代码
  return strides;
}
