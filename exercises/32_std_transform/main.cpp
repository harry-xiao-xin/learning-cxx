...
// READ: `std::transform`
// <https://zh.cppreference.com/w/cpp/algorithm/transform> READ:
// `std::vector::begin`
// <https://zh.cppreference.com/w/cpp/container/vector/begin>

int main(int argc, char **argv) {
  std::vector<int> val{8, 13, 21, 34, 55};
  // TODO: 调用 `std::transform`，将 `v` 中的每个元素乘以
  // 2，并转换为字符串，存入 `ans` std::vector<std::string> ans
  std::vector<std::string> ans;
  std::transform(val.begin(), val.end(), std::back_inserter(ans),
                 [](int item) { return std::to_string(item * 2); });
